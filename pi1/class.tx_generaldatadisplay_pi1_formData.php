...
abstract class tx_generaldatadisplay_pi1_formData {
			...
			switch ($check) {
				...
				case 'validImg':
					if (!preg_match('/^.+\.(jpeg|jpg|jpe|gif|tif|tiff|png|gif)$/i', $value)) {
						...
						if (is_file(FILEUPLOADPATH . "/" . $value))
							unlink(FILEUPLOADPATH . "/" . $value);
						$error[$check] = $check;
					} else
						$error[$check] = 0;
					break;
				...
				default:
					if (is_array($check))
						$error[key($check)] = preg_match($check[key($check)], $value) ? 0 : key($check);
					break;
			}
			// if notEmpty is not set empty values are ok
			if (!$value && $check != 'notEmpty')
				$error[$check] = 0;
		}
		return $error;
	}
	...
	public function valueExist($field, $value) {
		...
		foreach ($objArr as $key => $obj)
			if ($obj->getObjVar($field) == $value)
				return $obj->getObjVar('uid');
		
		return 0;
	}
	
	public function validFile($key) {
		if (!$_FILES[PREFIX_ID]['tmp_name'][$key]['select'])
			return FALSE;
		
		if (!$_FILES[PREFIX_ID]['error'][$key]['select'] && $_FILES[PREFIX_ID]['size'][$key]['select'] < MAXFILESIZE)
			return $_FILES[PREFIX_ID]['name'][$key]['select'];
		
		else {
			if ($_FILES[PREFIX_ID]['error'][$key]['select'])
				$this->formError[$key] = array(
					'fileUpload'
				);
			if ($_FILES[PREFIX_ID]['size'][$key]['select'] > MAXFILESIZE)
				$this->formError[$key] = array(
					'fileFilesize'
				);
			return FALSE;
...
class tx_generaldatadisplay_pi1_datafieldForm extends tx_generaldatadisplay_pi1_formData {
	...
	public function importValues(tx_generaldatadisplay_pi1_objVar $formData, tx_generaldatadisplay_pi1_objVar $secPiVars = NULL) {
		...
		$this->checkHash['uid']              = 'isInt';
		$this->checkHash['datafield_name']   = array(
			'notEmpty',
			'plainColumn'
		);
	...
	public function getMetadata($key = '') {
		$meta = $this->getFormValue('meta', TRUE) ? $this->getFormValue('meta', TRUE) : unserialize($this->formData->get('metadata', TRUE));
