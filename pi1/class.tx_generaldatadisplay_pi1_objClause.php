...
class tx_generaldatadisplay_pi1_objClause {
	...
	public function get($table) {
		...
		if ($andClause)
			$clauses[] = '(' . implode(' AND ', $andClause) . ')';
		if ($orClause)
			$clauses[] = '(' . implode(' OR ', $orClause) . ')';
		...
		foreach ($ruleArr[$key] as $index => $rule) {
			...
			if (!$error['plainColumn']) {
				...
				if (strtolower($rule['operator']) == 'in') {
					$clause .= ($clause ? " " . $rule['concat'] . " " : "") . "`" . $key . "`" . " " . $rule['operator'] . " " . '(' . $rule['value'] . ')';
					
				} else {
					$clause .= ($clause ? " " . $rule['concat'] . " " : "") . "`" . $key . "`" . " " . $rule['operator'] . " " . $GLOBALS['TYPO3_DB']->fullQuoteStr($rule['value'], $table);
				}
			}
		}
		return '(' . $clause . ')';
