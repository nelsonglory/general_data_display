...
abstract class tx_generaldatadisplay_pi1_dataFields extends tslib_pibase {
	...
	public function cleanMetadata(array &$metadata) {
		switch ($this->type) {
			case 'img':
				$keyArr = array(
					'datafield_searchable' => 'bool',
					'datafield_required' => 'bool',
				...
				if ($metadata['img_size_x'])
					$metadata['img_size_x'] = (int) $metadata['img_size_x'];
				if ($metadata['img_size_y'])
					$metadata['img_size_y'] = (int) $metadata['img_size_y'];
				if (!preg_match('/^(left|right|center)$/', $metadata['img_align']))
					unset($metadata['img_align']);
				break;
			...
			case 'currency':
				$keyArr = array(
					...
					'datafield_required' => 'bool',
					'content_visible' => 'bool',
					'default_value_prefix' => 'int',
					'default_value_suffix' => 'int',
...
class tx_generaldatadisplay_pi1_text extends tx_generaldatadisplay_pi1_dataFields {
	...
	public function HTML($type = 'edit') {
		...
		$this->tmplArr['###TOOLTIP###'] = $this->cObj->addParams($tooltip, array(
			'class' => 'tooltip'
		));
...
class tx_generaldatadisplay_pi1_img extends tx_generaldatadisplay_pi1_dataFields {
	// vars
	protected $type = "img";
	protected $config = array('subpartType' => array('edit' => '###IMAGE_INPUT###', 'config' => '###METADATA_IMAGE###'), 'imgAlign' => array('left' => 'left', 'center' => 'center', 'right' => 'right'));
}
