...
class tx_generaldatadisplay_pi1 extends tslib_pibase {
	...
	public function main($content, $conf) {
		...
		foreach ($this->secPiVars->get() as $key => $value) {
			switch (TRUE) {
				...
				case preg_match('/^(uid|selected_category|offset)$/', $key): {
					if (is_numeric($value))
						$this->secPiVars->setValue($key, intval($this->secPiVars->get($key)));
					else
						$this->secPiVars->delKey($key);
				}
					break;
		...
		foreach ($objArr as $key => $obj) {
			...
			$datafieldHash[$obj->getObjVar('uid')] = array(
				'name' => $obj->getObjVar('datafield_name', TRUE),
				'searchable' => ($metadata['datafield_searchable'] == 'yes' ? 'yes' : 'no')
			);
		}
		
		// add title to nameHash
		$datafieldHash['data_title'] = array(
			'name' => 'data_title',
			'searchable' => 'yes'
		);
		...
		if ($this->secPiVars->get('cancel'))
			$this->secPiVars->delKey('action');
				...
				if ($this->secPiVars->get('uid')) {
					...
					if ($this->secPiVars->get('direction') == 'up') {
						...
						foreach ($objArr as $key => $obj) {
							...
							if ($this->secPiVars->get('uid') == $objVars->get('uid'))
								break;
	...
	private function view() {
		if (!$this->secPiVars->get('view') && $this->conf['viewMode'] <= 2)
			$this->secPiVars->setValue('view', $this->conf['viewMode']);
		// get view
		...
		switch ($type) {
			case 'category': {
				if ($objArr) {
					...
					$contentAll['###TABLE_CONTENT###'] = '';
				$contentAll['###TABLE_HEADING###'] = $this->wrapInTag($this->getLL('categories'), __FUNCTION__ . '-heading');
				$createLink                        = $this->wrapInTag($this->pi_linkTP_keepPIvars('<img src="' . PICTURE_PATH . 'create.png" title="' . $this->getLL('new_category') . '" alt="[' . $this->getLL('new_category') . ']" />', array(
					'action' => 'update',
					'type' => 'category'
				), '1', '1'), __FUNCTION__ . '-admincreate');
			...
			case 'datafield': {
				...
				$createLink                        = $this->wrapInTag($this->pi_linkTP_keepPIvars('<img src="' . PICTURE_PATH . 'create.png" title="' . $this->getLL('new_datafield') . '" alt="[' . $this->getLL('new_datafield') . ']" />', array(
					'action' => 'update',
					'type' => 'datafield'
				), '1', '1'), __FUNCTION__ . '-admincreate');
				$contentAll['###ADMIN_CREATE###']  = ADM_PERM == 'BE' ? $createLink : '';
	...
	private function listView($type = 'data') {
		...
		switch ($type) {
			default: {
				...
				// include browselist
				$browseList = $this->makeBrowseList(array(
					'0-9',
					'A',
					'B',
					'C',
					...
					'Z',
					'all'
				), 'selected_letter', __FUNCTION__, '|', $this->secPiVars->get('selected_letter'));
				...
				$commons['###SEARCH_RESET###']     = $this->wrapInTag($this->pi_linkTP_keepPIvars('<img src="' . PICTURE_PATH . 'reset.png" title="' . $this->getLL('reset') . '" alt="[' . $this->getLL('reset') . ']" />', array(
					'search_reset' => 1
				), '1', '1', LIST_PID), 'optionfield-reset', '');
				...
				if ($nrPageResults && $nrPageResults < $nrResults) {
					...
					if ($from > 0)
						$contentArray['###PAGELINKS###'] = $this->wrapInTag($this->pi_linkTP_keepPIvars('<<', array(
							'offset' => $from - $nrPageResults,
							'type' => 'data'
						), '1', '1'), __FUNCTION__ . '-pageLink', '');
					if ($offset)
						$contentArray['###PAGELINKS###'] .= $this->wrapInTag($this->pi_linkTP_keepPIvars('<', array(
							'offset' => $offset - $nrPageResults,
							'type' => 'data'
						), '1', '1'), __FUNCTION__ . '-pageLink', '');
					for ($i = $from; $i < $to; $i = $i + $nrPageResults) {
						$index1 = $i;
						$index2 = ($i + $nrPageResults - 1) >= $nrResults ? $nrResults - 1 : $i + $nrPageResults - 1;
						$contentArray['###PAGELINKS###'] .= $this->secPiVars->get('offset') != $index1 ? $this->wrapInTag($this->pi_linkTP_keepPIvars('[' . ($index1 == $index2 ? ($index1 + 1) : ($index1 + 1) . '-' . ($index2 + 1)) . ']', array(
							'offset' => $index1,
							'type' => 'data'
						), '1', '1'), __FUNCTION__ . '-pageLink', '') : $this->wrapInTag('[' . ($index1 == $index2 ? ($index1 + 1) : ($index1 + 1) . '-' . ($index2 + 1)) . ']', __FUNCTION__ . '-pageLink-active', 'span');
					}
					if (($offset + $nrPageResults) < $nrResults)
						$contentArray['###PAGELINKS###'] .= $this->wrapInTag($this->pi_linkTP_keepPIvars('>', array(
							'offset' => $offset + $nrPageResults,
							'type' => 'data'
						), '1', '1'), __FUNCTION__ . '-pageLink', '');
					if ($index2 < $nrResults - 1)
						$contentArray['###PAGELINKS###'] .= $this->wrapInTag($this->pi_linkTP_keepPIvars('>>', array(
							'offset' => $index2 + 1,
							'type' => 'data'
						), '1', '1'), __FUNCTION__ . '-pageLink', '');
		...
		switch ($type) {
			case 'data': {
				...
				if ($objArr[$uid]) {
					...
					return $this->showError('dberror_no_dataset');
				$contentArrayCSS = $this->wrapTemplateArrayInClass($contentArray, __FUNCTION__);
				$contentAll      = array_merge($contentArrayCSS, $commonsArray);
				$content         = $this->cObj->substituteMarkerArrayCached($subpart, $contentAll);
			}
	...
	private function editView(tx_generaldatadisplay_pi1_formData &$formData) {
		// check ADM_PERM
		if (!ADM_PERM)
			return $this->showError('error_noPermission');
					...
					if ($datafieldTypesArr[$objVars->get('datafield_type')]) {
						...
						switch ($objVars->get('datafield_type')) {
							...
							case 'currency':
								$dataFieldContent = $formData->getFormValue($objVars->get('datafield_name'));
								if (!$dataFieldContent)
									$dataFieldContent = array(
										'VALUE_PREFIX' => $metadata['default_value_prefix'],
										'VALUE_SUFFIX' => $metadata['default_value_suffix'],
										'CURRENCY' => $metadata['default_currency']
									);
						...
						foreach ($ferror as $key => $value) {
							if ($value) {
								$formErrorDatafield .= $this->wrapInTag(($this->getLL('error_' . $value) ? $this->getLL('error_' . $value) : 'error_' . $value), __FUNCTION__ . '-formError');
							}
						}
	...
	private function importData() {
		...
		if (!$this->getConfigValue('allowDataImport', 'bool', FALSE) || ADM_PERM != 'BE')
			return $this->showError('error_noPermission');
		// reset form ?
		if ($this->secPiVars->get('reset_import')) {
		...
		// is mapping file submitted
		if ($this->secPiVars->get('submit_mapping') && $_FILES[PREFIX_ID]['tmp_name']['mappingfile']) {
			$data = file_get_contents($_FILES[PREFIX_ID]['tmp_name']['mappingfile']);
			// check mapping data
			if (is_array(unserialize($data)))
				$this->sessionData('mapData', $data, TRUE);
			else
				return $this->showError('error_fileMimeType');
		}
		// save mapping ?
		if ($this->secPiVars->get('save_mapping')) {
			$file = t3lib_div::makeInstance(PREFIX_ID . '_exportMapping');
			// set data ...
			$file->setData($this->sessionData('mapData'), 'GDDmapping');
			// .. and export!
			$file->export();
		}
		// do the import
		if ($this->secPiVars->get('submit_import')) {
			...
			// check title
			if (!$hasTitle)
				return $this->showError('Das Mapping eines Titels ist notwendig!');
			// start importing
			$importError = FALSE;
			...
			// get datafieldlist to find datafield type
			$dataFieldList = t3lib_div::makeInstance(PREFIX_ID . '_datafieldList');
			$dataFieldList->getDS();
			$datafieldTypeHash = $dataFieldList->getHash('datafield_type');
			...
			for ($id = $startLine - 1; $id < $importLines; $id++) {
				...
				foreach ($dataSet as $key => $value) {
					$dataFieldName = $mapData['dataFields']['init'][$mappingHash[$key]];
					$dataFieldType = $datafieldTypeHash[$mappingHash[$key]];
					
					if ($dataFieldName) {
						// replace \r with \n to import linbreaks correctly
						$value = strip_tags(htmlspecialchars_decode($value, ENT_QUOTES), $value);
						// import routine for special fields
						switch ($dataFieldType) {
							...
							case 'date':
								...
								// check
								if (tx_generaldatadisplay_pi1_formData::checkValue($hash, 'isDate'))
									$value = $hash;
							break;
							...
							case 'time':
								...
								// check
								if (tx_generaldatadisplay_pi1_formData::checkValue($hash, 'isTime'))
									$value = $hash;
							...
							case 'currency':
								...
								// clean value
								$value_prefix = preg_replace('/(\.|,)/','',$matches[1]);
								
								// build currency hash
								$hash = array(
									'VALUE_PREFIX' => $value_prefix,
									'VALUE_SUFFIX' => $matches[3],
									'CURRENCY' => ''
								);
								
								// check
								if (tx_generaldatadisplay_pi1_formData::checkValue($hash, 'isCurrency'))
									$value = $hash;
							
							break;
							
						}
						$objVars->setValue($dataFieldName, $value);
					}
				}
				// todo check title ...
			...
			// add mapping
			if (($this->secPiVars->get('mapping_add') && $this->secPiVars->get('map_selected_CSVField') != NULL && $this->secPiVars->get('map_selected_dataField') != NULL) || $this->secPiVars->get('submit_mapping')) {
				// save mapping
				$mapData = unserialize($this->sessionData('mapData'));
				
				if (!$this->secPiVars->get('submit_mapping')) {
					$csvField  = $this->secPiVars->get('map_selected_CSVField');
					$dataField = $this->secPiVars->get('map_selected_dataField');
					
					$mapData['mapping'][$csvField . ':' . $dataField] = $mapData['CSVFields']['unmapped'][$csvField] . ' => ' . $mapData['dataFields']['unmapped'][$dataField];
					$this->sessionData('mappingFields', serialize($mapping), TRUE);
					// remove from session
					unset($mapData['CSVFields']['unmapped'][$csvField]);
					unset($mapData['dataFields']['unmapped'][$dataField]);
					// ... and save
					$this->sessionData('mapData', serialize($mapData), TRUE);
				}
			}
			...
			if ($importError)
				$content .= $this->showError($this->getLL('error_importLines') . ':\n' . wordwrap(implode(', ', $importError), 30, '\n', TRUE), FALSE);
		}
		return $content;
	}
	...
	private function exportData() {
		// do nothing without permission
		if (!$this->getConfigValue('allowDataExport', 'bool', FALSE) || ADM_PERM != 'BE')
			return $this->showError('error_noPermission');
		// instantiate datalist 
		$dataList = t3lib_div::makeInstance(PREFIX_ID . '_dataList');
		$objArr   = $dataList->getDS($this->searchClause, '', TRUE);
	...
	private function isAdmin() {
		...
		if ($GLOBALS['BE_USER']->user) {
			...
			if ($GLOBALS['BE_USER']->doesUserHaveAccess($dataArray, 2))
				return 'BE';
		}
	...
	private function sessionData($varName, $varContent = NULL, $set = FALSE) {
		...
		if ($set) {
			...
			return $GLOBALS['TSFE']->fe_user->getKey('ses', $this->extKey . '-' . DATA_PID . '-' . $varName);
	}
	...
	private function getOptionsFromArr(array $optionArr, $selected = '', $locale = FALSE) {
		...
		foreach ($optionArr as $key => $value) {
			...
			if ($value) {
				$optionEntry = '<option value="' . $key . '"' . (($key == $selected) ? ' selected="selected">' : '>') . $value . '</option>';
	...
	private function getTableFromArr($arr, $class = '', $numbered = TRUE) {
		...
		if ($class)
			$table = $this->wrapInTag($table, $class);
		return $table;
	}
		...
		switch ($type) {
			case 'date':
				...
				if (is_array($dataContent)) {
					if (isset($dataContent['MONTH']) && !$dataContent['MONTH'])
						unset($dataContent['MONTH']);
					if (isset($dataContent['DAY']) && !$dataContent['DAY'])
						unset($dataContent['DAY']);
			...
			case 'time':
				...
				if (is_array($dataContent)) {
					...
					if (!$dataContent['SECOND'])
						unset($dataContent['SECOND']);
					$content = implode(':', $dataContent);
				}
				break;
			...
			case 'file':
				$content = $this->pi_linkTP_keepPIvars($content, array(
					'action' => 'download',
			...
			case 'img':
				if ($obj) {
					...
					$imgSizeArr = array();
					if ($metadata['img_size_x'])
						$imgSizeArr[] = 'width="' . $metadata['img_size_x'] . '"';
					if ($metadata['img_size_y'])
						$imgSizeArr[] = 'height="' . $metadata['img_size_y'] . '"';
					
					$content = '<div ' . ($metadata['img_align'] ? 'style="text-align:' . $metadata['img_align'] . '"' : '') . '>' . $this->pi_linkTP_keepPIvars('<img src="' . FILEUPLOADPATH . '/' . md5($content) . '" alt="' . $content . '" ' . implode(' ', $imgSizeArr) . ' />', array(
						'action' => 'download',
						'selected_file' => $content
					), 0) . '</div>';
				}
					...
					while (preg_match($regex, $content, $matches)) {
						switch ($regex) {
							case '/\{{2}([^\{\}]*)\}{2}/':
								...
								break;
							case '/\[{2}(([^\[]|\[[^\[])*)\|(([^\[]|\[[^\[])*)\]{2}/':
								$replace = $this->cObj->typoLink($matches[3], array(
									'parameter' => $matches[1],
									'extTarget' => '_blank'
								));
								break;
							...
							case '/\[{2}(([^\[]|\[[^\[])*)\]{2}/':
								$replace = $this->cObj->typoLink($matches[1], array(
									'parameter' => $matches[1],
									'extTarget' => '_blank'
								));
								break;
	...
	private function makeCommonsArray() {
		...
		$commonsArray['###RESET###']        = $this->getLL('reset');
		$commonsArray['###SAVE_MAPPING###'] = $this->getLL('save_mapping');
		$commonsArray['###LOAD_MAPPING###'] = $this->getLL('load_mapping');
	...
	private function makeHeadingsArray() {
		...
		$headingsArray['###HEADING_IMPORT_OTHER_FUNCTIONS###'] = $this->getLL('import_other_functions');
	...
	private function makeOptionArray() {
		...
		$optionArray['###SELECTED_ITEM_OPTIONS###'] = '<option value="0">' . $this->getLL('all_items') . '</option>
								  <option value="data_title"' . ($this->secPiVars->get('selected_item') == 'data_title' ? ' selected="selected"' : '') . '>' . $this->pi_getLL('title') . '</option>' . $datafieldList->getOptionSelect('datafield_name', $this->secPiVars->get('selected_item'));
	...
	private function makeBrowseList($array, $getVar, $class, $separator = '|', $active = '') {
		$linkArray = array();
		foreach ($array as $l) {
			$linkArray[] = ($l == $active) ? $this->wrapInTag($this->getLL($l), $class . '-browselist-link-active', 'span') : $this->wrapInTag($this->pi_linkTP_keepPIvars($this->getLL($l), array(
				$getVar => $l
			), '1', '1', LIST_PID), $class . '-browselist-link', '');
		}
	...
	private function makeAdminStuff($uid, $type = 'data') {
		if ($uid && (ADM_PERM == 'BE' || (ADM_PERM == 'FE' && $type == 'data'))) {
			$stuff = $this->pi_linkTP_keepPIvars('<img src="' . PICTURE_PATH . 'edit.png" title="' . $this->getLL('modify') . '" alt="[' . $this->getLL('modify') . ']" />', array(
				'uid' => $uid,
				'action' => 'update',
				'type' => $type
			), '1', '1');
			$stuff .= $this->pi_linkTP_keepPIvars('<img src="' . PICTURE_PATH . 'trash.png" title="' . $this->getLL('delete') . '" alt="[' . $this->getLL('delete') . ']" />', array(
				'uid' => $uid,
			...
			if ($type == 'datafield' && ADM_PERM == 'BE') {
				$stuff .= $this->pi_linkTP_keepPIvars('<img src="' . PICTURE_PATH . 'toggle_down.png" title="' . $this->getLL('entry_down') . '" alt="[' . $this->getLL('entry_down') . ']" />', array(
					'uid' => $uid,
					'action' => 'update-sequence',
					'type' => $type,
					'direction' => 'down'
				), '1', '1');
				$stuff .= $this->pi_linkTP_keepPIvars('<img src="' . PICTURE_PATH . 'toggle_up.png" title="' . $this->getLL('entry_up') . '" alt="[' . $this->getLL('entry_up') . ']" />', array(
					'uid' => $uid,
					'action' => 'update-sequence',
					'type' => $type,
					'direction' => 'up'
	...
	private function makeAdminLinks() {
		if (ADM_PERM) {
			...
			$adminMenu[] = $this->pi_linkTP_keepPIvars('<img src="' . PICTURE_PATH . 'create.png" title="' . $this->getLL('new_data') . '" alt="[' . $this->getLL('new_data') . ']" />', array(
				'action' => 'update',
				'type' => 'data'
			), '1', '1');
			if (ADM_PERM == 'BE') {
				$adminMenu[] = $this->pi_linkTP_keepPIvars('<img src="' . PICTURE_PATH . 'categories.png" title="' . $this->getLL('categories') . '" alt="[' . $this->getLL('categories') . ']" />', array(
					'type' => 'category',
					'view' => '3'
				), '1', '1');
				$adminMenu[] = $this->pi_linkTP_keepPIvars('<img src="' . PICTURE_PATH . 'datafields.png" title="' . $this->getLL('datafields') . '" alt="[' . $this->getLL('datafields') . ']" />', array(
					'type' => 'datafield',
					'view' => '3'
				), '1', '1');
				if ($this->getConfigValue('allowDataExport', 'bool', FALSE))
					$adminMenu[] = $this->pi_linkTP_keepPIvars('<img src="' . PICTURE_PATH . 'export_data.png" title="' . $this->getLL('export_data') . '" alt="[' . $this->getLL('export_data') . ']" />', array(
						'action' => 'export'
					), '1', '1');
				if ($this->getConfigValue('allowDataImport', 'bool', FALSE))
					$adminMenu[] = $this->pi_linkTP_keepPIvars('<img src="' . PICTURE_PATH . 'import_data.png" title="' . $this->getLL('import_data') . '" alt="[' . $this->getLL('import_data') . ']" />', array(
						'action' => 'import'
					), '1', '1');
			}
	...
	private function getConfigValue($configVar, $type = 'string', $defaultValue = '', $flexformSection = FALSE) {
		$configValue = $flexformSection ? $this->pi_getFFvalue($this->cObj->data['pi_flexform'], $configVar, $flexformSection) : $this->conf[$configVar];
	...
	private function wrapInTag($str, $class, $tag = 'div') {
		if ($str) {
			...
			return $this->cObj->addParams($str, array(
				'class' => $class
			));
		} else
			return '';
